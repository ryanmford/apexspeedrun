...
const continents = {
  ...
  "af": ["ALGERIA", "ANGOLA", "BENIN", "BOTSWANA", "BURKINA FASO", "BURUNDI", "CABO VERDE", "CAMEROON", "CENTRAL AFRICAN REPUBLIC", "CHAD", "COMOROS", "CONGO", "DJIBOUTI", "EGYPT", "EQUATORIAL GUINEA", "ERITREA", "ESWATINI", "ETHIOPIA", "GABON", "GAMBIA", "GHANA", "GUINEE", "GUINEA-BISSAU", "IVORY COAST", "KENYA", "LESOTHO", "LIBERIA", "LIBYA", "MADAGASCAR", "MALAWI", "MALI", "MAURITANIA", "MAURITIUS", "MOROCCO", "MOZAMBIQUE", "NAMIBIA", "NIGER", "NIGERIA", "RWANDA", "SAO TOME AND PRINCIPE", "SENEGAL", "SEYCHELLES", "SIERRA LEONE", "SOMALIA", "SOUTH AFRICA", "SOUTH SUDAN", "SUDAN", "TANZANIA", "TOGO", "TUNISIA", "UGANDA", "ZAMBIA", "ZIMBABWE"],
...
const CustomStyles = () => (
  <style>{`
    ...
    .glow-gold { text-shadow: 0 0 10px rgba(234, 179, 8, 0.5); }
    .glow-silver { text-shadow: 0 0 10px rgba(161, 161, 170, 0.4); }
    .glow-bronze { text-shadow: 0 0 10px rgba(206, 137, 70, 0.5); }
    .glow-fire { text-shadow: 0 0 10px rgba(239, 68, 68, 0.6); }
    ...
    .dark-tooltip .leaflet-popup-content-wrapper,
    ...
    .light-tooltip .leaflet-popup-content-wrapper,
    .light-tooltip .leaflet-tooltip {
      background: white;
      color: #18181b;
...
const ASRTooltip = ({ text, children, direction = 'top', align = 'center' }) => {
  const posClass = direction === 'top' ? 'bottom-full mb-2' : 'top-full mt-2';
  const animClass = direction === 'top' ? 'translate-y-1 group-hover/tooltip:-translate-y-0' : '-translate-y-1 group-hover/tooltip:translate-y-0';
  ...
  return (
    <div className="relative group/tooltip inline-flex items-center justify-center cursor-help">
      {children}
      <div className={`absolute ${posClass} ${alignClass} z-[500] w-[180px] sm:w-56 p-2.5 text-[10px] sm:text-xs font-bold normal-case tracking-normal leading-relaxed rounded-xl bg-black/90 backdrop-blur-xl text-slate-200 border border-white/10 shadow-2xl pointer-events-none opacity-0 invisible group-hover/tooltip:opacity-100 group-hover/tooltip:visible ${animClass} transition-all duration-300 max-w-[calc(100vw-2rem)]`}>
        {text}
        {direction === 'top' && (
        ...
        {direction === 'bottom' && (
            <div className="absolute bottom-full left-1/2 -translate-x-1/2 border-4 border-transparent border-b-white/10">
                <div className="absolute top-full left-1/2 -translate-x-1/2 border-4 border-transparent border-b-black/90 -translate-y-[3px]" />
            </div>
        )}
      </div>
    </div>
...
const processSetListData = (csv) => {
    ...
    const seasonIdx = findIdx(['season', 'open']);
    ...
    lines.slice(1).forEach(l => {
        ...
        if (course) {
            ...
            const valSeason = seasonIdx !== -1 ? String(vals[seasonIdx] || "").toUpperCase().trim() : "";
            const valDate = dateIdx !== -1 ? String(vals[dateIdx] || "").toUpperCase().trim() : "";
            
            const agMarked = valAG === 'YES' || valAG === 'TRUE' || valAG === 'X' || valAG === '1' || valAG.includes('2026') || valAG.includes('OPEN');
            const seasonMarked = valSeason.includes('2026') || valSeason.includes('OPEN') || valSeason === 'YES';
            const dateMarked = valDate.includes('2026');

            map[course] = { 
                is2026: agMarked || seasonMarked || dateMarked, 
                flag: fixed.flag || '🏳️',
...
const processSettersData = (csv) => {
    ...
    const leadsIdx = headers.findIndex(h => h === 'lead' || h === 'leads' || h === 'lead sets');
    const assistsIdx = headers.findIndex(h => h === 'assist' || h === 'assists' || h === 'assistant' || h === 'assistant sets');
    const setsIdx = headers.findIndex(h => h === 'sets' || h === 'total sets' || h === 'set');

    const countryIdx = headers.findIndex(h => h === 'country' || h === 'nation');
    const flagIdx = headers.findIndex(h => h === 'flag' || h === 'emoji' || h === 'region');
    const igIdx = headers.findIndex(h => h === 'ig' || h === 'instagram' || h === 'social');
                    ...
                    <div key={c.name} onClick={() => onCourseClick(c)} className={`group flex items-center justify-between p-3 sm:p-4 rounded-lg sm:rounded-xl border transition-all cursor-pointer active:scale-[0.98] ${theme === 'dark' ? 'bg-white/5 border-white/5 hover:bg-white/10' : 'bg-white border-slate-300/50 shadow-sm hover:bg-slate-50'}`}>
                        <div className="flex items-center gap-3 pr-4 min-w-0">
                            {c.coordinates ? (
                                <a 
                                    ...
                                    className={`p-2 rounded-full transition-all hover:bg-blue-500/10 hover:text-blue-500 ${theme === 'dark' ? 'text-white/40' : 'text-slate-400'}`}
                                >
                                    <IconCourse />
                                </a>
                            ) : (
                            ...
                            <div className="flex flex-col min-w-0">
                                <span className={`text-xs sm:text-base font-mono font-black uppercase whitespace-normal break-words transition-colors group-hover:text-blue-500 ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>{c.name}</span>
                        ...
                        <div className="flex gap-4 sm:gap-8 items-center shrink-0">
                            <div className="flex flex-col items-end">
                                ...
                                <span className="text-xs sm:text-sm font-mono font-bold text-blue-500">{c.totalRuns || c.totalAthletes || 0}</span>
                            </div>
                            ...
                            <div className="w-8 flex justify-end shrink-0">
                                {c.demoVideo && (
                                    <a href={c.demoVideo} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} className={`p-1.5 rounded-full transition-colors hover:bg-black/10 dark:hover:bg-white/10 ${theme === 'dark' ? 'text-blue-400' : 'text-blue-500'}`}>
...
const ASRLocationModal = ({ isOpen, onClose, onBack, onForward, canGoForward, data, type, theme, courses, onCourseClick, breadcrumbs, onBreadcrumbClick }) => {
    ...
    const Header = (
        <div className="flex items-center gap-4 sm:gap-6 min-w-0 w-full pr-2">
            <div className={`w-16 h-16 sm:w-28 sm:h-28 rounded-2xl sm:rounded-[2rem] border flex items-center justify-center text-blue-500 shrink-0 shadow-xl ${theme === 'dark' ? 'bg-black/30 border-white/10' : 'bg-white/5 border-slate-300'}`}>
        ...
        <ASRBaseModal isOpen={isOpen} onClose={onClose} onBack={onBack} onForward={onForward} canGoForward={canGoForward} theme={theme} header={Header} breadcrumbs={breadcrumbs} onBreadcrumbClick={onBreadcrumbClick}>
            <div className="space-y-2 sm:space-y-3 mb-6 sm:mb-8">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3">
                    {stats.map((s, i) => (
                        <div key={i} className={`flex flex-col border p-3 sm:p-5 rounded-xl ${theme === 'dark' ? 'bg-white/5 border-white/5' : 'bg-white border-slate-300/50 shadow-sm'}`}>
...
const ASRProfileModal = ({ isOpen, onClose, onBack, onForward, canGoForward, identity, initialRole, theme, allCourses, playerPerformances, openModal, breadcrumbs, onBreadcrumbClick, updateCurrentHistory }) => {
    ...
    const Header = (
        <div className="flex items-center gap-4 sm:gap-6 min-w-0 w-full pr-2">
            <div className={`w-16 h-16 sm:w-28 sm:h-28 rounded-2xl sm:rounded-[2rem] border flex items-center justify-center text-2xl sm:text-5xl font-black shadow-xl shrink-0 uppercase overflow-hidden relative ${theme === 'dark' ? 'bg-black/30 border-white/10 text-slate-500' : 'bg-white/5 border-slate-300 text-slate-500'}`}>
                {!imgError ? (
    ...
    const renderPlayerContent = () => {
        ...
        const playerStats = [
            { l: 'RATING', v: (identity.rating || 0).toFixed(2), c: 'text-blue-500' }, 
            ...
            { l: 'AVG TIME', v: (identity.avgTime || 0).toFixed(2), tip: 'Average finish time across all courses run.' },
            { l: '🪙', v: identity.contributionScore || 0, g: 'glow-gold', tip: 'Coin bonus for runs, sets, & wins.' }, 
            { l: '🔥', v: totalFires, g: 'glow-fire', tip: 'Fire bonus for course breaker runs.' }
        ];
            ...
            <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3 mb-8">
                    {playerStats.map((s, i) => {
                        ...
                        return (
                            <div key={i} className={`flex flex-col border p-3 sm:p-5 rounded-xl transition-all ${theme === 'dark' ? 'bg-white/5 border-white/5' : 'bg-white border-slate-300/50 shadow-sm'}`}>
                                <span className={`text-[10px] sm:text-xs font-black uppercase tracking-[0.1em] mb-1 sm:mb-2 flex items-center`}>
                                    <span className={`${isEmoji ? 'opacity-100' : 'opacity-50'} flex items-center gap-1.5`}>
                                        {s.tip ? (
                                            <ASRTooltip text={s.tip} direction="bottom" align={i % 2 === 0 ? 'left' : 'right'}>
                                                <span className="cursor-help border-b border-dashed border-current">{isEmoji ? '' : s.l}</span>
                                            </ASRTooltip>
                                        ) : (!isEmoji && s.l)}
                ...
                <h3 className={`text-[10px] sm:text-xs font-black uppercase tracking-[0.15em] px-1 sm:px-2 mb-3 ${theme === 'dark' ? 'text-slate-500' : 'text-slate-600'}`}>COURSES RUN</h3>
                <div className="grid grid-cols-1 gap-2">
                    {courseData.map((c, i) => (
                        <div key={i} onClick={() => { const target = allCourses.find(x => x.name.toUpperCase() === c.label.toUpperCase()); if(target) openModal('course', target); }} className={`group flex items-center justify-between p-3 sm:p-4 rounded-lg sm:rounded-xl border transition-all cursor-pointer active:scale-[0.98] ${theme === 'dark' ? 'bg-white/5 border-white/5 hover:bg-white/10' : 'bg-white border-slate-300/50 shadow-sm hover:bg-slate-50'}`}>
                            <div className="flex items-center gap-3 pr-3 min-w-0">
                                ...
                                <div className="flex flex-col min-w-0">
                                    <span className={`text-xs sm:text-base font-mono font-black uppercase whitespace-normal break-words transition-colors group-hover:text-blue-500 ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>{c.label}</span>
                            ...
                            <div className="flex items-center gap-4 shrink-0">
                                ...
                                <div className="w-8 flex justify-end shrink-0">
                                    {c.videoUrl && (
                                        <a href={c.videoUrl} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} className="p-1.5 rounded-full text-blue-500 hover:bg-black/5 dark:hover:bg-white/5">
                                            <IconVideoPlay size={16} />
                                        </a>
                                    )}
                                </div>
    ...
    const renderSetterContent = () => {
        ...
        const setterStatsGrid = [
            { l: 'IMPACT', v: setterData.impact || 0, c: 'text-blue-500', tip: 'Total runs on courses set.' },
                    ...
                    {setterStatsGrid.map((s, i) => (
                        <div key={i} className={`flex flex-col border p-3 sm:p-5 rounded-xl ${theme === 'dark' ? 'bg-white/5 border-white/5' : 'bg-white border-slate-300/50 shadow-sm'}`}>
                            <span className="text-[10px] sm:text-xs font-black uppercase tracking-wider mb-1 sm:mb-2 opacity-50 flex items-center">
                                {s.tip ? <ASRTooltip text={s.tip} direction="bottom" align={i % 2 === 0 ? 'left' : 'right'}><span className="cursor-help border-b border-dashed border-current">{s.l}</span></ASRTooltip> : s.l}
                            </span>
                            <span className={`text-xs sm:text-xl font-mono font-black num-col ${s.c || ''}`}>{s.v}</span>
                        </div>
                    ))}
                </div>
                <h3 className={`text-[10px] sm:text-xs font-black uppercase tracking-[0.15em] px-1 sm:px-2 mb-3 ${theme === 'dark' ? 'text-slate-500' : 'text-slate-600'}`}>COURSES SET</h3>
                {setterCourses.length > 0 ? (
                    <ASRProfileCourseList courses={setterCourses} theme={theme} onCourseClick={(c) => openModal('course', c)} preFiltered={true} />
                ) : (
    ...
    return (
        <ASRBaseModal isOpen={isOpen} onClose={onClose} onBack={onBack} onForward={onForward} canGoForward={canGoForward} theme={theme} header={Header} breadcrumbs={breadcrumbs} onBreadcrumbClick={onBreadcrumbClick}>
            {isBoth && (
                <div className={`flex p-1 rounded-xl mb-6 border w-fit mx-auto sm:mx-0 ${theme === 'dark' ? 'bg-black/40 border-white/10' : 'bg-slate-300/50 border-slate-400/20'}`}>
                    <button onClick={() => handleRoleSwitch('player')} className={`px-4 py-1.5 rounded-lg text-[10px] sm:text-xs font-black uppercase tracking-widest transition-all ${activeRole === 'player' ? 'bg-blue-600 text-white shadow-lg' : 'opacity-50 hover:opacity-100'}`}>PLAYER STATS</button>
                    <button onClick={() => handleRoleSwitch('setter')} className={`px-4 py-1.5 rounded-lg text-[10px] sm:text-xs font-black uppercase tracking-widest transition-all ${activeRole === 'setter' ? 'bg-blue-600 text-white shadow-lg' : 'opacity-50 hover:opacity-100'}`}>SETTER STATS</button>
                </div>
            )}
...
const ASRCourseModal = ({ isOpen, onClose, onBack, onForward, canGoForward, course, theme, athleteMetadata, athleteDisplayNameMap, onPlayerClick, onSetterClick, breadcrumbs, onBreadcrumbClick }) => {
    ...
    const Header = (
        <div className="flex items-center gap-4 sm:gap-6 min-w-0 w-full pr-2">
            <div className={`w-16 h-16 sm:w-28 sm:h-28 rounded-2xl sm:rounded-[2rem] border flex items-center justify-center text-blue-500 shrink-0 shadow-xl ${theme === 'dark' ? 'bg-black/30 border-white/10' : 'bg-white/5 border-slate-300'} `}><IconCourse size={32} /></div>
            <div className="flex flex-col min-w-0 justify-center">
                ...
                onEachFeature: (feature, layer) => {
                    ...
                    if (countryData) {
                        layer.on({
                            mouseover: (e) => {
                                const l = e.target;
                                l.setStyle({ fillColor: '#3b82f6', fillOpacity: 0.15, color: '#3b82f6', opacity: 0.6, weight: 2 });
    ...
    if (!loaded) {
        return (
            <div className={`w-full h-[60vh] sm:h-[75vh] flex flex-col items-center justify-center rounded-2xl sm:rounded-3xl border shadow-xl ${theme === 'dark' ? 'bg-white/5 border-white/10 text-white' : 'bg-slate-100 border-slate-300 text-slate-900'}`}>
                <div className="animate-spin opacity-50 mb-4"><IconSpeed /></div>
    ...
    return (
        <div className={`relative w-full h-[60vh] sm:h-[75vh] min-h-[500px] rounded-2xl sm:rounded-3xl overflow-hidden shadow-2xl border ${theme === 'dark' ? 'dark-zoom border-white/10' : 'light-zoom border-slate-300'}`}>
            ...
            <div className="absolute top-4 right-4 z-[40] flex flex-col items-end gap-1.5 pointer-events-none w-[65%] sm:w-auto">
                <div className={`px-3 py-1.5 sm:px-4 sm:py-2 rounded-xl sm:rounded-2xl border backdrop-blur-xl shadow-lg text-[10px] sm:text-xs font-black uppercase tracking-widest flex items-center justify-between gap-3 sm:gap-4 w-fit ${theme === 'dark' ? 'bg-black/80 border-white/10 text-slate-200' : 'bg-white/90 border-slate-300 text-slate-800'}`}>
                ...
                {eventType === 'all-time' && !warningDismissed && (
                    <div onClick={() => setWarningDismissed(true)} className={`pointer-events-auto cursor-pointer px-2.5 py-1.5 rounded-lg backdrop-blur-md shadow-sm text-[9px] sm:text-[10px] font-bold text-right leading-tight flex items-center gap-1.5 sm:gap-2 transition-all hover:opacity-80 active:scale-95 ${theme === 'dark' ? 'bg-slate-800/90 text-slate-300 border border-white/10' : 'bg-white/90 text-slate-600 border border-slate-200'}`}>
                        <span>Includes courses not in the ASR Open.</span>
                        <span className="opacity-50 flex items-center justify-center shrink-0"><IconX size={12} /></span>
                    </div>
                )}
            </div>
            ...
            <div className="absolute top-4 left-4 z-[40] flex flex-col gap-2.5 pointer-events-none w-[calc(100%-2rem)] max-w-xs h-[calc(100%-5rem)] sm:h-auto">
                <button onClick={() => setIsPanelOpen(!isPanelOpen)} className={`pointer-events-auto w-fit px-4 py-3 sm:px-5 sm:py-3.5 rounded-xl sm:rounded-2xl border backdrop-blur-xl shadow-xl transition-all flex items-center gap-2 text-[10px] sm:text-xs font-black uppercase tracking-widest ${theme === 'dark' ? 'bg-black/80 border-white/10 text-white hover:bg-black' : 'bg-white/90 border-slate-300 text-slate-900 hover:bg-white'}`}>
                            ...
                            return (
                                <div key={i} onClick={handleClick} className={`cursor-pointer flex items-center justify-between p-2 sm:p-2.5 rounded-lg sm:rounded-xl border border-transparent transition-all ${theme === 'dark' ? 'hover:bg-white/10' : 'hover:bg-slate-200/50'}`}>
                                    <div className="flex items-center gap-2 sm:gap-3 min-w-0 pr-2">
                                        <div className="scale-75 sm:scale-90 origin-left shrink-0"><ASRRankBadge rank={i + 1} theme={theme} /></div>
            ...
            <div className="p-3 sm:p-4 border-b border-inherit bg-inherit flex items-center justify-between rounded-t-2xl sm:rounded-t-3xl">
                <h4 className="text-[10px] sm:text-xs font-black uppercase tracking-wider flex items-center gap-1.5 flex-wrap">
                    {sec.l.split(' ').map((word, wi) => {
                        ...
                        const text = isFire ? "Fire bonus for course breaker runs (men: sub-9, women: sub-11)." : isGold ? "Coin bonus for runs, sets, & wins." : isImpact ? "Total runs on courses set." : "";
                        const content = <span key={wi} className={highlight}>{word}</span>;
                        return text ? <ASRTooltip key={wi} text={text} direction="bottom" align="center">{content}</ASRTooltip> : content;
                    })}
                </h4>
            </div>
                      ...
                      <div key={`${sec.k}-${i}-${p.name}`} className={`group flex items-center justify-between p-2.5 sm:p-4 hover:bg-white/[0.03] transition-colors gap-2 ${['cityStats', 'countryStats'].includes(sec.k) ? '' : 'cursor-pointer active:scale-[0.98]'}`} onClick={() => {
                        ...
                        <div className="flex items-center gap-1.5 sm:gap-2.5 min-w-0 pr-1">
                          ...
                          <div className="flex flex-col ml-0.5">
                            <span className={`text-xs sm:text-sm font-black uppercase whitespace-normal break-words leading-tight ${!['cityStats', 'countryStats'].includes(sec.k) ? 'group-hover:text-blue-500' : ''} transition-colors`}>{p.name}</span>
          ...
          <table className="hof-table min-w-full relative">
            <thead className={`sticky top-0 z-20 backdrop-blur-xl shadow-sm ${theme === 'dark' ? 'bg-[#121214]/95 text-slate-400' : 'bg-white/95 text-slate-500'}`}>
              <tr className={`text-[10px] sm:text-xs font-black uppercase tracking-widest`}>
                ...
                <ASRHeaderComp l="TOTAL" k="total" a="right" w="w-12 sm:w-24 lg:w-32 pr-2 sm:pr-8" activeSort={medalSort} handler={setMedalSort} theme={theme} />
              </tr>
            </thead>
            <tbody className={`divide-y ${theme === 'dark' ? 'divide-white/5' : 'divide-slate-200'}`}>
              {stats.medalCount.map((c) => (
                <tr key={`medal-row-${c.name}-${c.flag}`} className="group hover:bg-black/[0.02] transition-colors">
                  ...
                  <td className="px-2 py-3 sm:py-8 w-full min-w-[100px]">
                    <div className="flex flex-col">
                      <span className={`text-xs sm:text-[15px] font-black uppercase whitespace-normal break-words leading-tight block ${tColor}`}>{c.name}</span>
                  ...
                  <td className={`pr-2 sm:pr-8 text-right font-mono font-black ${tColor} text-xs sm:text-[15px] tabular-nums num-col`}>{c.total}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
const ASRNavBar = ({ theme, setTheme, view, setView }) => {
    const navItems = [{id:'map',l:'MAP'}, {id:'players',l:'PLAYERS'}, {id:'setters',l:'SETTERS'}];
    return (
        <nav className={`fixed top-0 w-full backdrop-blur-xl border-b z-50 h-14 sm:h-20 flex items-center justify-between px-4 sm:px-8 lg:px-12 transition-all duration-500 ${theme === 'dark' ? 'bg-[#09090b]/90 border-white/5' : 'bg-[#cbd5e1]/85 border-slate-400/30'}`}>
            {/* Left Section: Logo - flex-shrink-0 ensures it doesn't compress Nav Items */}
            <div className="flex items-center gap-2 shrink-0">
                <div className={`${theme === 'dark' ? 'text-blue-400' : 'text-blue-500'} animate-pulse`}><IconSpeed /></div>
                <span className="font-black tracking-tighter text-xs sm:text-2xl uppercase italic leading-none transition-all whitespace-nowrap hidden xs:block">
                    ASR <span className="hidden sm:inline">APEX SPEED RUN</span>
                </span>
            </div>

            {/* Center Section: Navigation Buttons - flex-grow and justify-center spreads them out */}
            <div className="flex-1 flex justify-center items-center px-4 overflow-visible">
                <div className="flex items-center gap-1.5 sm:gap-4 md:gap-8 w-full justify-center">
                    {navItems.map(v => (
                        <button key={v.id} onClick={() => setView(v.id)} className={`flex-1 sm:flex-none border px-2 sm:px-6 md:px-8 py-1.5 sm:py-2.5 rounded-full text-[10px] sm:text-xs font-black uppercase tracking-widest transition-all duration-300 whitespace-nowrap select-none ${view === v.id ? 'bg-blue-600 border-blue-600 text-white shadow-[0_0_15px_rgba(37,99,235,0.5)] scale-105' : (theme === 'dark' ? 'border-white/10 text-slate-400 hover:border-white/30 hover:text-white' : 'border-slate-400/30 text-slate-500 hover:border-slate-400 hover:text-slate-700 bg-white/20')}`}>
                            {v.l}
                        </button>
                    ))}
                </div>
            ...
            {/* Right Section: Theme Toggle - flex-shrink-0 ensures it doesn't compress Nav Items */}
            <div className="shrink-0">
                <button aria-label="Toggle Theme" onClick={() => setTheme(t => t === 'dark' ? 'light' : 'dark')} className={`w-8 h-8 sm:w-12 sm:h-12 flex items-center justify-center border rounded-full transition-all ${theme === 'dark' ? 'bg-black/40 border-white/10 text-slate-400 hover:text-white' : 'bg-slate-300/50 border-slate-400/20 text-slate-600 hover:text-black'}`}>
...
const ASRControlBar = ({ view, setView, eventType, setEventType, gen, setGen, search, setSearch, theme }) => {
    const titles = { players: 'PLAYERS', setters: 'SETTERS', map: 'MAP', hof: 'HALL OF FAME' };
    return (
        <header className={`pt-20 sm:pt-24 pb-6 sm:pb-8 px-4 sm:px-8 max-w-7xl mx-auto w-full flex flex-col gap-4 sm:gap-10 bg-gradient-to-b ${theme === 'dark' ? 'from-blue-600/10' : 'from-blue-500/5'} to-transparent`}>
            <h1 className={`text-4xl sm:text-6xl font-black uppercase tracking-tighter ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>{titles[view] || 'ASR'}</h1>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
                <div className={`flex items-center p-0.5 sm:p-1 rounded-xl border w-fit h-fit shrink-0 ${theme === 'dark' ? 'bg-black/40 border-white/10' : 'bg-slate-300/50 border-slate-400/20'}`}>
                    <div className="flex flex-wrap gap-0.5">
                        <button onClick={() => { setEventType('open'); if(view === 'hof') setView('map'); }} className={`px-3 sm:px-6 py-2 sm:py-2.5 rounded-lg sm:rounded-xl text-xs sm:text-sm font-black uppercase tracking-widest transition-all whitespace-nowrap ${view !== 'hof' && eventType === 'open' ? 'bg-blue-600 text-white shadow-lg scale-105' : 'text-slate-500 hover:text-slate-700 hover:bg-black/5 dark:hover:text-slate-300 dark:hover:bg-white/5'}`}>ASR OPEN</button>
                        <button onClick={() => { setEventType('all-time'); if(view === 'hof') setView('map'); }} className={`px-3 sm:px-6 py-2 sm:py-2.5 rounded-lg sm:rounded-xl text-xs sm:text-sm font-black uppercase tracking-widest transition-all whitespace-nowrap ${view !== 'hof' && eventType === 'all-time' ? 'bg-blue-600 text-white shadow-lg scale-105' : 'text-slate-500 hover:text-slate-700 hover:bg-black/5 dark:hover:text-slate-300 dark:hover:bg-white/5'}`}>ALL-TIME</button>
                        <button onClick={() => setView('hof')} className={`px-3 sm:px-6 py-2 sm:py-2.5 rounded-lg sm:rounded-xl text-xs sm:text-sm font-black uppercase tracking-widest transition-all whitespace-nowrap ${view === 'hof' ? 'bg-blue-600 text-white shadow-lg scale-105' : 'text-slate-500 hover:text-slate-700 hover:bg-black/5 dark:hover:text-slate-300 dark:hover:bg-white/5'}`}>HOF</button>
                    </div>
                </div>
            ...
            {eventType === 'open' && view !== 'hof' && (
                <div className={`flex flex-col gap-4 sm:gap-6 w-full animate-in fade-in slide-in-from-top-4 duration-700 mb-2 sm:mb-4`}>
                    <div className={`flex flex-col items-center justify-center py-10 sm:py-16 px-4 rounded-2xl sm:rounded-[2rem] border relative overflow-hidden ${theme === 'dark' ? 'bg-blue-600/10 border-blue-500/20 shadow-[0_0_40px_rgba(37,99,235,0.15)]' : 'bg-blue-50 border-blue-200 shadow-xl'}`}>
                        ...
                        <h4 className={`mb-6 sm:mb-8 text-xs sm:text-sm font-black uppercase tracking-[0.3em] sm:tracking-[0.5em] animate-subtle-pulse drop-shadow-lg ${theme === 'dark' ? 'text-blue-400' : 'text-blue-600'}`}>THE 2026 ASR OPEN STARTS IN:</h4>
                        <div className="scale-90 sm:scale-100 drop-shadow-xl"><CountdownTimer targetDate={new Date('2026-03-01T00:00:00-10:00')} theme={theme} /></div>
                    </div>
                    <div className={`p-6 sm:p-8 rounded-2xl sm:rounded-[2rem] border flex flex-col md:flex-row gap-6 items-center justify-between ${theme === 'dark' ? 'bg-black/40 border-white/10' : 'bg-white border-slate-300'} shadow-lg`}>
                        <div className="flex-1 space-y-1.5 text-center md:text-left">
                            <h3 className={`text-sm sm:text-lg font-black uppercase tracking-widest ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>GET ON BOARD</h3>
                            <p className={`text-xs sm:text-sm font-bold leading-relaxed opacity-80 max-w-2xl ${theme === 'dark' ? 'text-slate-300' : 'text-slate-700'}`}>Join our free online community app to get official ASR details, submit your runs, and get verified. No fees required. Just proof of work.</p>
                        </div>
                        <a href="https://www.skool.com/apexmovement/about" target="_blank" rel="noopener noreferrer" className="shrink-0 flex items-center justify-center gap-2.5 px-6 py-3.5 sm:px-8 sm:py-4 bg-blue-600 hover:bg-blue-500 text-white text-xs sm:text-sm font-black uppercase tracking-widest rounded-xl transition-all shadow-lg w-full md:w-auto">
                            JOIN THE GAME <IconCornerUpRight size={16} />
                        </a>
                    </div>
                </div>
            )}
            ...
            {view !== 'hof' && view !== 'map' && (
                <div className="w-full relative group">
                    <div className={`absolute left-3.5 top-1/2 -translate-y-1/2 transition-opacity ${theme === 'dark' ? 'text-slate-600' : 'text-slate-400'} group-focus-within:text-blue-500`}><IconSearch size={12} /></div>
                    ...
                    {search && (
                        <button aria-label="Clear Search" onClick={() => setSearch('')} className={`absolute right-3.5 top-1/2 -translate-y-1/2 p-1 rounded-lg hover:bg-black/10 transition-colors ${theme === 'dark' ? 'text-white/40 hover:text-white' : 'text-slate-400 hover:text-slate-600'}`}><IconX size={14} /></button>
                    )}
                </div>
            )}
        </header>
...
const SETTERS_COLS = [
    ...
    { label: 'IMPACT', type: 'highlight', key: 'impact', align: 'right', width: 'w-14 sm:w-32', tooltip: 'Total runs on courses set.' },
...
const COURSE_COLS = [
    ...
    { label: 'RECORDS', type: 'records', key: 'mRecord', align: 'right', width: 'w-16 sm:w-40 pr-4 sm:pr-10' }
];
...
export default function App() {
  ...
  useEffect(() => {
    try { window.localStorage.setItem('asr-theme', theme); } catch(e) {}
  }, [theme]);
  ...
  const handleSort = (newSort) => {
    ...
    setViewSorts(prev => ({ ...prev, [view === 'map' ? 'courses' : view]: updated }));
  };
  ...
  const list = useMemo(() => {
    ...
    if (finalQual.length > 0 && finalUnranked.length > 0) {
        return [...finalQual, { isDivider: true, label: gen === 'M' ? "RUN 4+ COURSES TO GET RANKED" : "RUN 2+ COURSES TO GET RANKED" }, ...finalUnranked];
    }
  ...
  const rawCourseList = useMemo(() => {
    ...
    return courseNames.map(name => {
      ...
      return {
        ...
        athletesM: athletesMAll, athletesF: athletesFAll, parsedCoords, ...meta
      };
  ...
  const settersWithImpact = useMemo(() => {
    return settersData.map(s => {
        ...
        let computedLeads = 0, computedAssists = 0;
        sCourses.forEach(c => {
            if ((c.leadSetters || "").toLowerCase().includes(sNameLower)) computedLeads++;
            if ((c.assistantSetters || "").toLowerCase().includes(sNameLower)) computedAssists++;
        });
        return { 
            ...s, impact: sCourses.reduce((sum, c) => sum + (c.totalRuns || 0), 0),
            sets: isAllTimeContext ? Math.max(s.sets || 0, sCourses.length) : sCourses.length,
            leads: isAllTimeContext ? Math.max(s.leads || 0, computedLeads) : computedLeads,
            assists: isAllTimeContext ? Math.max(s.assists || 0, computedAssists) : computedAssists
        };
    }).filter(s => isAllTimeContext || s.sets > 0);
  }, [settersData, rawCourseList, isAllTimeContext]);

  const settersList = useMemo(() => {
    if (view !== 'setters') return []; 
    const filtered = settersWithImpact.filter(s => s.name.toLowerCase().includes(debouncedSearch.toLowerCase()));
    const qualified = filtered.filter(s => s.sets >= 3), unranked = filtered.filter(s => s.sets < 3);
    const sort = viewSorts.setters;
    const dir = sort.direction === 'ascending' ? 1 : -1;
    const sortFn = (a, b) => robustSort(a, b, sort.key, dir) || (b.sets - a.sets);
    ...
    return (finalQual.length && finalUnranked.length) ? [...finalQual, { isDivider: true, label: "SET 3+ COURSES TO GET RANKED" }, ...finalUnranked] : [...finalQual, ...finalUnranked];
  ...
  const cityList = useMemo(() => {
    ...
    const dir = sort.direction === 'ascending' ? 1 : -1;
    return base.filter(c => c.name.toLowerCase().includes(debouncedSearch.toLowerCase())).sort((a, b) => robustSort(a, b, sort.key, dir)).map((c, i) => ({ ...c, currentRank: i + 1 }));
  }, [rawCourseList, viewSorts.cities, debouncedSearch]);
  ...
  const countryList = useMemo(() => {
    ...
    const dir = sort.direction === 'ascending' ? 1 : -1;
    return base.filter(c => c.name.toLowerCase().includes(debouncedSearch.toLowerCase())).sort((a, b) => robustSort(a, b, sort.key, dir)).map((c, i) => ({ ...c, currentRank: i + 1 }));
  }, [rawCourseList, viewSorts.countries, debouncedSearch]);

  const continentList = useMemo(() => {
    const base = calculateContinentStats(rawCourseList);
    return base.filter(c => c.name.toLowerCase().includes(debouncedSearch.toLowerCase())).sort((a, b) => b.courses - a.courses).map((c, i) => ({ ...c, currentRank: i + 1 }));
  }, [rawCourseList, debouncedSearch]);
  ...
  const renderActiveModal = () => {
    ...
    switch (activeModal.type) {
        ...
        case 'city': return <ASRLocationModal {...modalProps} data={activeModal.data} type='city' courses={rawCourseList} onCourseClick={(c) => openModal('course', c)} />;
        case 'country': return <ASRLocationModal {...modalProps} data={activeModal.data} type='country' courses={rawCourseList} onCourseClick={(c) => openModal('course', c)} />;
        case 'continent': return <ASRLocationModal {...modalProps} data={activeModal.data} type='continent' courses={rawCourseList} onCourseClick={(c) => openModal('course', c)} />;
        default: return null;
    }
  };
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 font-sans pb-24 select-none flex flex-col antialiased ${theme === 'dark' ? 'bg-[#09090b] text-slate-200' : 'bg-[#cbd5e1] text-slate-900'}`}>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-8 flex-grow w-full relative">
        ...
        {isLoading ? <ASRSkeletonTable theme={theme} /> : hasError && data.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 sm:py-32 space-y-6">
                <div className="p-6 rounded-full bg-red-500/10 text-red-500 mb-4"><IconX size={32} /></div>
                <div className={`text-xs sm:text-sm font-black uppercase tracking-[0.2em] text-center px-4 ${theme === 'dark' ? 'text-slate-300' : 'text-slate-700'}`}>CONNECTION TO ASR SERVERS FAILED</div>
                <button onClick={() => window.location.reload()} className="mt-8 px-6 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-[10px] sm:text-xs font-black uppercase tracking-widest transition-all shadow-lg">RETRY CONNECTION</button>
            </div>
        ) : view === 'hof' ? (
          ...
          <div className="space-y-8">
            ...
            <div className={`relative border rounded-xl sm:rounded-3xl shadow-2xl overflow-hidden flex flex-col ${theme === 'dark' ? 'bg-black/20 border-white/5' : 'bg-white border-slate-300'}`}>
              ...
              <div className="overflow-auto scrollbar-hide max-h-[75vh] relative w-full">
                {(view === 'map' ? courseList.length : view === 'players' ? list.length : settersList.length) > 0 ? (
                  <ASRDataTable 
                      theme={theme}
                      columns={view === 'map' ? COURSE_COLS : view === 'players' ? PLAYER_COLS : SETTERS_COLS}
                      sort={viewSorts[view === 'map' ? 'courses' : view]}
                      onSort={handleSort}
                      data={view === 'map' ? courseList : view === 'players' ? list : settersList}
                      onRowClick={(item) => openModal(view === 'map' ? 'course' : view === 'players' ? 'player' : 'setter', item)}
                  />
                ) : (
                  <div className={`flex flex-col items-center justify-center py-24 sm:py-32 text-center px-4 ${theme === 'dark' ? 'text-white/40' : 'text-slate-900/40'}`}>
                      <div className="scale-[2] sm:scale-[2.5] mb-8 sm:mb-12 animate-pulse opacity-40"><IconSpeed /></div>
                      <h3 className="text-sm sm:text-xl font-black uppercase tracking-widest mb-1.5 sm:mb-2 text-current">LEADERBOARDS GO LIVE AT LAUNCH</h3>
                      <p className="text-[10px] sm:text-xs font-black uppercase tracking-[0.3em] opacity-60">CHECK BACK SOON</p>
                  </div>
                )}
              </div>
            </div>
