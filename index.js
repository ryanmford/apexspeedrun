...
      --bottom-nav-height: 64px; /* Slightly tighter for Instagram look */
    }
    
    html, body {
      margin: 0;
...
    /* Updated single marker style using SVG pin */
    .asr-marker-pin-wrap {
      color: #2563eb;
...
      {norm === 'MASTER' && <span className="text-blue-200 animate-pulse text-[10px]">✨</span>}
    </div>
  );
};
...
              </div>
              <div className="flex items-center gap-2">
                <button onClick={handleShare} className="p-3.5 sm:p-3 bg-black/30 hover:bg-black/50 rounded-full text-white transition-all shrink-0" title="Share Link">
...
            { l: 'IMPACT', v: String(Math.round(impact)) },
...
                      <div 
                        key={idx} 
                        className="py-14 px-10 sm:px-12 text-center opacity-60 text-[11px] font-black uppercase tracking-[0.5em] transition-all duration-300 hover:opacity-100 hover:bg-blue-600/5 hover:scale-[1.005] cursor-default border-y border-transparent hover:border-blue-600/10"
                      >
                        {item.label}
                      </div>
                    );
...
      <div className={`w-full px-6 pt-3 pb-[calc(10px+var(--safe-bottom))] border-t transition-all duration-500 flex items-center justify-around ${theme === 'dark' ? 'bg-black/95 border-zinc-800 text-slate-100 shadow-[0_-10px_40px_rgba(0,0,0,0.8)]' : 'bg-white/95 border-slate-200 shadow-[0_-10px_30px_rgba(0,0,0,0.05)]'} backdrop-blur-2xl`}>
        {items.map(item => (
          <button
            key={item.id}
            onClick={() => {
...
             <h1 className={`${theme === 'dark' ? 'text-white' : 'text-slate-900'} text-xl sm:text-2xl font-black uppercase tracking-[0.3em] text-left px-1 drop-shadow-md`}>
                {currentTitle}
             </h1>
          </div>
